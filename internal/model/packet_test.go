...
-----END OpenVPN Static key V1-----`
	...
	pa := &PacketAuth{LocalDigestKey: &local, RemoteDigestKey: &remote, Mode: ControlAuthModeTLSAuth}
...
-----END OpenVPN Static key V1-----`
	pa, err := NewTLSCryptPacketAuth(string(authData))
	if err != nil {
		t.Error(err)
	}
	got, err := SerializePacket(packet, pa)
	if err != nil {
		t.Error(err)
	}
